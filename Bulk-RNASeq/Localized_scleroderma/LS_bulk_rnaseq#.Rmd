...
packages <- c("dplyr", "limma", "ggplot2", "ggrepel", "ComplexHeatmap", "Rtsne", "RColorBrewer", "clusterProfiler", "fgsea", "msigdbr","pathview", "org.Hs.eg.db", "enrichplot")
...
```{r}
raw_counts <- read.csv('./GSE166861_FPKM_normalized_data.csv.gz')
...
results <- topTable(fit2, coef = 1, sort.by = "P", n = Inf)
...
# remove rows where ID starts with "LOC" - since they can be artifacts 
results_filtered <- results[!grepl("^LOC", results$ID), ]

# convert 'regulated' to a factor with levels in a meaningful order
results_filtered$regulated <- factor(results_filtered$regulated, 
                                           levels = c("up", "down", "not significant"))

# summarize the number of up/down/not significant genes
...
min_val <- min(significant_mat)
max_val <- max(significant_mat)
mid_val <- 0

# Custom color palette from darkred to white to green
col_fun <- colorRampPalette(c("darkred", "white", "green"))

# Scale matrix
scaled_mat <- t(scale(t(significant_mat)))

# Generate a simple heatmap
heatmap <- Heatmap(scaled_mat,
...
# Scale matrix and create the heatmap annotation
scaled_mat <- t(scale(t(significant_mat)))
col_annot <- HeatmapAnnotation(df = sample.info,
                               col = list(condition = c("Healthy"= "red", "LS" = "blue")))
...
heatmap <- Heatmap(scaled_mat,
                   ...
                   bottom_annotation = HeatmapAnnotation(
                     counts = anno_barplot(colSums(scaled_mat)),
...
heatmap <- Heatmap(t(scale(t(significant_mat))),
                   top_annotation = col_annot,
                   show_row_names = FALSE,
                   name = "Differentially expressed genes across samples")
...
# Create gene_list before using it
gene_list <- significant_genes$logFC
names(gene_list) <- significant_genes$ID
